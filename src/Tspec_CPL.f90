subroutine spc(N,Ne,NB1,NB2,NB3,NB,B1,B2,B3,IBCT,BCT,maxv,maxvct,TDMe,TDMm,R,OM0,&
&          SF,SC,SA,E,C,T,NP,Emin,Emax,sigma,func,maxS,outp,shift,emi, &
&          ECDvel,CD,redem,Iosc,stick)
implicit none
...
double precision, dimension(Ne,3) :: TDMm
...
double precision, dimension(:,:,:), allocatable :: MTDMD
...
logical :: twopemi, emi,redem,Iosc,stick,CD, ECDvel
...
if ( CD .eqv. .true. ) then
        call create_emisionTDM(N,Ne,maxv,maxvct,NB1,NB2,NB3,NB,NGS,NGS1,NGS2,B1,B2,B3,GS1,GS2,IBCT,BCT,MTDMD,TDMm,SF,SC,SA)
end if 
print*, "Extended ground state TDMs... OK"
call flush(6)
...
if ( CD .eqv. .true. ) then
     ...
     do j=0,NGS
       call CD_abs(NB,ETDMD(j,:,:),MTDMD(j,:,:),C,AE(j,:),WETDM(j,:),R,ECDvel)
     end do
   print*, "CPL intensity... OK"
   call flush(6)
  ...
  do i=1,NB
    do j=0,NGS
      ...
      EE(k)=AE(j,i)
      F(k)=WETDM(j,i)*W(i)
      print*, k,i,j, EE(k),F(k),W(i),WETDM(j,i)
    end do
  end do
